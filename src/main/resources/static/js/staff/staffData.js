            ...
            case 'update':
                if (data.length === 0) {
                    ...
                    parent.layer.open({
                        ...
                        title: '编辑内部职工',
                        area: ['30%', '250px'],
                        content: ['gotoUpdateStaff', 'no'],//跳到后台
                        success: function (layer, index) {
                            // 获取子页面的iframe
                            var iframe = parent.window['layui-layer-iframe' + index];
                            iframe.getdata(data[0].id, data[0].name,data[0].phone,data[0].position);
                        },
                        end: function () {
                            tableIns.reload();
                        }
        ...
        if (layEvent === 'del') {
            ...
            parent.layer.open({
                ...
                success: function (layer, index) {
                    // 获取子页面的iframe
                    var iframe = parent.window['layui-layer-iframe' + index];
                    iframe.getdata(data.id, data.name,data.phone,data.position);
                },
                end: function () {
                    tableIns.reload();
                }
